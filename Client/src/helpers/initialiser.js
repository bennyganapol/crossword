...
export const getChallenges = async () => {
  let res = await axios.post(
    `${window.location.protocol}//${window.location.hostname}:4000/games`,
  );
  ...
  challenges.forEach((challenge) => {
    ...
    if (challenge.questionX != null && challenge.questionY != null) {
      ...
      if (questionSquare) {
        ...
        challenge.arrowType = getChallengeArrowType(
          challenge.questionX,
          challenge.questionY,
          challenge.x,
          challenge.y,
    ...
    if (challenge.imageX != null && challenge.imageY != null) {
      ...
      if (imageSquare) {
        ...
        if (
          challenge.questionX !== challenge.imageX &&
          challenge.questionY &&
          challenge.imageX
        ) {
...
const trySortSquareChallenges = (questionSquare) => {
  if (questionSquare.challenges && questionSquare.challenges.length > 1) {
    ...
    if (
      arrowTypeOrder[questionSquare.challenges[0].arrowType] >
      arrowTypeOrder[questionSquare.challenges[1].arrowType]
    ) {
      questionSquare.challenges = [
        questionSquare.challenges[1],
        questionSquare.challenges[0],
      ];
    }
  }
};
