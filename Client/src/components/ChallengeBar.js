...
function ChallengeBar(props) {
  ...
  const getWidth = () => {
    ...
    if (
      props.selectedChallenge.multi &&
      props.selectedChallenge.multi.length >= 2
    ) {
  ...
  const renderSquares = () => {
    ...
    if (
      ...
      for (let i = 1; i < props.selectedChallenge.multi; i++) {
        spacesIndexes[i] =
          spacesIndexes[i - 1] + props.selectedChallenge.multi[i];
      }
    }
  ...
  return (
    <div style={{ padding: '10px' }}>
      <div style={{ textAlign: 'right', height: '35px', fontSize: '25px' }}>
        {props.selectedChallenge.question}
      </div>
      <div
        style={{
          display: 'flex',
          ...
          marginLeft: 'auto',
          flexDirection:
            props.horizontalDirection === 'rtl' ? 'row-reverse' : 'row',
          width: getWidth() + 'px',
          height: '60px',
        }}
      >
